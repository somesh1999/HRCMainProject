...
class CustomerInvoices extends Component {
  ...
  render(){ 
            ...
            const handleSelectAllClick = (event) => {
                ...
                const newSelectedrowss = this.state.responseData.slice(0,1000);
            ...
            const rowCount= this.state.responseData.length;
            const order = this.state.order;
            const orderBy = this.state.orderBy;

            const buttonDisabled = this.state.buttonDisabled;
            const buttonDisabledExport = this.state.buttonDisabledExport;
            const headCells = [
            ...
            const openModal = () =>{
              ...
              const modifydetails = () => {
                          axios.post(`http://localhost:8080/1705745/modifydetails`,
                              ...
                              {
                                  headers: { "Content-Type": "application/json" },
                                  params: { indacctheader: this.state.indacctheader, indopenamount: this.state.indopenamount, inddoctype: this.state.inddoctype },
                              }
                              )
                              .then((response) => {
                                  handleClose();
                ...
                const handlechange1=(event) =>{
                    this.setState({
                      indopenamount : event.target.value
                    })
                }
                const handlechange2=(event) =>{
                    this.setState({
                      inddoctype : event.target.value
                    })
                }

                const result = Array.from(this.state.headerResponse).map(a => a.total_open_amount);
                const result1 = Array.from(this.state.headerResponse).map(a => a.total_open_invoice);
                const total_open_amount =(result[0]/1000).toFixed(2);
                const total_open_invoice = result1[0];
                                                      ...
                                                      <Grid item xs={12} style={{marginTop: "-10px"}}>
                                                          <Grid container spacing={0}>
                                                              <Grid item xs={12} sm={4}>
                                                                  <Typography variant="h6" component="h6" style={{fontSize:"14px", color:"rgb(166, 172, 175)", width:"100%"}}>Document Type </Typography>
                                                              </Grid>
